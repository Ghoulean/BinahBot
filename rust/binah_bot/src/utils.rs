...
pub fn get_option_value<'a>(
    option_name: &'a str,
    options: &'a [DiscordInteractionOptions],
) -> Option<&'a DiscordInteractionOptionValue> {
    options
        .iter()
...
pub fn get_focused_option(
    options: &[DiscordInteractionOptions],
) -> Option<&DiscordInteractionOptions> {
        ...
        env.locales.lookup_with_args(
            ...
            &HashMap::from([
                (
                    "display",
                    FluentValue::from(
                        get_display_name_locale(parsed_typed_id, locale)
                            .unwrap_or(parsed_typed_id.to_string()),
                    ),
                ),
...
pub fn get_display_name_locale(typed_id: &ParsedTypedId, locale: &Locale) -> Option<String> {
    match typed_id.0 {
        PageType::AbnoPage => get_abno_page_locales_by_internal_name(&typed_id.1)
            .get(locale)
            .map(|x| x.card_name.to_string()),
        PageType::BattleSymbol => get_battle_symbol_locales_by_internal_name(&typed_id.1)
            .get(locale)
            .map(|x| format!("{} {}", x.prefix, x.postfix)),
        PageType::CombatPage => get_combat_page_locales_by_id(&typed_id.1)
            .get(locale)
            .map(|x| x.name.to_string()),
        PageType::KeyPage => get_key_page_by_id(&typed_id.1).and_then(|key_page| {
            key_page
                .text_id
                .map(|text_id| {
                    get_key_page_locales_by_text_id(text_id)
                        .get(locale)
                        .map(|key_page_locale| key_page_locale.name.to_string())
                })
                .or(key_page.skin.map(|skin| Some(skin.to_string())))
                .flatten()
        }),
        PageType::Passive => get_passive_locales_by_id(&typed_id.1)
            .get(locale)
            .map(|x| x.name.to_string()),
    }
}
...
pub fn build_error_message_response(
    lang_id: &LanguageIdentifier,
    err_code: &str,
    env: &BinahBotEnvironment,
) -> MessageResponse {
...
pub fn build_delete_button_component(
    lang_id: &LanguageIdentifier,
    env: &BinahBotEnvironment,
) -> ButtonComponent {
    ButtonComponent {
...
mod tests {
    ...
    fn sanity_get_focused_option() {
        ...
        assert_eq!(options[1].name, get_focused_option(&options).unwrap().name);
    }
    ...
    fn check_no_lookup_display_collisions() {
        Locale::iter().for_each(|locale: Locale| {
            ...
            let abno_page_ids = get_all_abno_pages()
                .iter()
                .map(|x| ParsedTypedId(PageType::AbnoPage, x.internal_name.to_string()))
                .collect::<Vec<_>>();
            let battle_symbol_ids = get_all_battle_symbols()
                .iter()
                .map(|x| ParsedTypedId(PageType::BattleSymbol, x.internal_name.to_string()))
                .collect::<Vec<_>>();
            let combat_page_ids = get_all_combat_pages()
                .iter()
                .map(|x| ParsedTypedId(PageType::CombatPage, x.id.to_string()))
                .collect::<Vec<_>>();
            let keypage_ids = get_all_key_pages()
                .iter()
                .map(|x| ParsedTypedId(PageType::KeyPage, x.id.to_string()))
                .collect::<Vec<_>>();
            let passive_ids = get_all_passives()
                .iter()
                .map(|x| ParsedTypedId(PageType::Passive, x.id.to_string()))
                .collect::<Vec<_>>();
            ...
            abno_page_ids
                ...
                .for_each(|x| {
                    ...
                    let fmt = format!(
                        ...
                        disambig_entry.cloned().unwrap_or("")
                    );
                    lookups
                        .entry(fmt)
