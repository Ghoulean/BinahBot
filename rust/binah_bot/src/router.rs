...
pub fn get_response(discord_interaction: &DiscordInteraction, binahbot_env: &BinahBotEnvironment) -> DiscordInteractionResponse {
    ...
    match (&discord_interaction.r#type, &discord_interaction.data) {
        ...
        (DiscordInteractionType::ApplicationCommand, Some(data)) if data.name == CommandName::Lor.to_string() => {
            DiscordInteractionResponse::MessageResponse(lor_command(discord_interaction, binahbot_env))
        }
