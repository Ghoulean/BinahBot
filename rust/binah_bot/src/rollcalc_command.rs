...
pub fn rollcalc_command(interaction: &DiscordInteraction, env: &BinahBotEnvironment) -> MessageResponse {
    let binding = vec![];
    ...
    let min1 = get_option_value("min1", command_args).map(|x| match x {
        DiscordInteractionOptionValue::Integer(y) => y,
        _ => unreachable!()
    }).expect("no min1");
    let min2 = get_option_value("min2", command_args).map(|x| match x {
        DiscordInteractionOptionValue::Integer(y) => y,
        _ => unreachable!()
    }).expect("no min2");
    let max1 = get_option_value("max1", command_args).map(|x| match x {
        DiscordInteractionOptionValue::Integer(y) => y,
        _ => unreachable!()
    }).expect("no max1");
    let max2 = get_option_value("max2", command_args).map(|x| match x {
        DiscordInteractionOptionValue::Integer(y) => y,
        _ => unreachable!()
    }).expect("no max2");
            ...
            fields: Some(vec![
                DiscordEmbedFields {
                    ...
                    value: env.locales.lookup_with_args(
                        ...
                        "win_format",
                        &HashMap::from([
                            ("percentage", format_percent((calc.1 as f64) / (calc.0 as f64))),
                ...
                DiscordEmbedFields {
                    ...
                    value: env.locales.lookup_with_args(
                        ...
                        "win_format",
                        &HashMap::from([
                            ("percentage", format_percent((calc.2 as f64) / (calc.0 as f64))),
                ...
                DiscordEmbedFields {
                    ...
                    value: env.locales.lookup_with_args(
                        ...
                        "win_format",
                        &HashMap::from([
                            ("percentage", format_percent((calc.3 as f64) / (calc.0 as f64))),
...
fn calculate(min1: i32, max1: i32, min2: i32, max2: i32) -> DiceDistribution {
    ...
    let overlap: u128 = (cmp::max(0, cmp::min(max1, max2) - cmp::max(min1, min2) + 1)).try_into().unwrap();
...
mod tests {
    ...
    fn performance_calculate() {
        ...
        assert_eq!(DiceDistribution(
            4294967296, 2147450880, 2147450880, 65536
        ), result);
    }
