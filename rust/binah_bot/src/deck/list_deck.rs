...
pub async fn list_deck(
    ...
    let incomplete_name = get_option_value("name", command_args)
        .and_then(|x| cast_enum_variant!(x, DiscordInteractionOptionValue::String));
    let keypage_option = get_option_value("keypage", command_args)
        .and_then(|x| cast_enum_variant!(x, DiscordInteractionOptionValue::String));
    let author_id_option = get_option_value("author", command_args)
        .and_then(|x| cast_enum_variant!(x, DiscordInteractionOptionValue::String));
    ...
    let choices = match focused {
        Some(x) => match x {
            "keypage" => get_choices_by_keypage_query(keypage_option, &card_locale, &lang_id, env),
            _ => {
...
pub async fn list_my_decks(
    ...
    let deck_name_query = get_option_value("name", command_args)
        .and_then(|x| cast_enum_variant!(x, DiscordInteractionOptionValue::String));
    ...
    tracing::info!(
        "Got the following: incomplete_name={:?}, author_id={:?}, focused={:?}",
        deck_name_query,
        author_id,
        focused
    );
...
async fn get_choices_by_deck_name(
    ...
    match decks {
        Ok(results) => get_top_matches(&query, &results, 10)
            .into_iter()
            .map(|x| {
...
fn get_top_matches<'a>(
    query: &'a str,
    decks: &'a Vec<DeckMetadata>,
    top: usize,
) -> Vec<&'a DeckMetadata> {
    let map: HashMap<usize, &String> = decks
        .iter()
        .enumerate()
        .map(|(i, x)| (i, &x.name))
        .collect();
...
mod tests {
    ...
    fn sanity_get_top_matches() {
        let decks = vec![
            DeckMetadata {
                name: "Turbo Nikolai".to_string(),
                author_id: "1".to_string(),
                author_name: "gh".to_string(),
            },
            DeckMetadata {
                name: "Bore Kali to death with this one neat trick!".to_string(),
            ...
            DeckMetadata {
                ...
                author_name: "ai".to_string(),
            },
            DeckMetadata {
                name: "Crimson Claws".to_string(),
                author_id: "4".to_string(),
                author_name: "eb".to_string(),
            },
            DeckMetadata {
                name: "TurboNikolai".to_string(),
                author_id: "1".to_string(),
                author_name: "gh".to_string(),
            },
        ];
        let top = get_top_matches("turbo nikolai", &decks, 2)
            .iter()
            .map(|x| x.name.as_str())
            .collect::<Vec<_>>();
        dbg!(&top);
        ...
        let top = get_top_matches("one neat trick", &decks, 1)
            .iter()
            .map(|x| x.name.as_str())
            .collect::<Vec<_>>();
        assert_eq!(1, top.len());
        assert_eq!(
            Some(&"Bore Kali to death with this one neat trick!"),
            top.get(0)
        );

        let top = get_top_matches("CCCCCCCC", &decks, 10)
            .iter()
            .map(|x| x.name.as_str())
            .collect::<Vec<_>>();
        assert_eq!(1, top.len());
