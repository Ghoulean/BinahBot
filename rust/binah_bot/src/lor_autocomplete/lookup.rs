...
pub fn get_typed_ids_from_query(query: &str) -> Vec<TypedId<'static>> {
    ...
    let mut typed_ids = tokens.iter().filter_map(|x| {
        get_typed_ids(&x.0)    
    }).map(|x| set_to_hashmap_count(&x))
        .fold(
            HashMap::new(), 
            |hm1, hm2| {
                combine_hashmap_count(&hm1, &hm2)
            })
        .into_iter()
        .collect::<Vec<_>>();
    typed_ids.sort_by(|a, b| b.1.cmp(&a.1));
    typed_ids.clone().into_iter().map(|(x, _)| x.clone()).collect()
}

fn set_to_hashmap_count<'a>(hs: &HashSet<&'a TypedId<'static>>) -> HashMap<&'a TypedId<'static>, i32> {
    hs.iter().map(|x| {
        (*x, 1)
    }).collect()
}
...
fn combine_hashmap_count<'a>(
    ...
    let combined: HashMap<_, _> = all_keys.into_iter().map(|x| {
        (*x, hm1.get(x).unwrap_or(&0) + hm2.get(x).unwrap_or(&0))
    }).collect();
    combined
}
