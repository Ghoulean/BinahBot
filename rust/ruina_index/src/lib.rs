...
pub fn get_disambiguation_page<'a>(display_name: &'a str, locale: &'a Locale) -> Option<&'a DisambiguationPage<'static>> {
    match locale {
...
pub fn get_autocomplete_entry<'a>(typed_id: &'a TypedId, locale: &'a Locale) -> Option<&'a Autocomplete<'static>> {
...
mod tests {
    ...
    fn sanity_get_typed_ids() {
        ...
        let pillar_hashset = get_typed_ids("pillar")
            .expect("pillar hashset not found");
        assert!(pillar_hashset.get(&degraded_pillar).is_some());
        // pillar, degraded pillar
        assert_eq!(pillar_hashset.len(), 2);

        let degraded_hashset = get_typed_ids("degrad")
            .expect("degrad hashset not found");
