...
pub fn get_disambiguations_for_uniqueness_heuristic<'a>(
    ...
    // locale -> vec<ids>
    let disambiguations = Locale::iter()
        .map(|x| {
            (
                x.clone(),
    ...
    for (locale, vec_ids) in disambiguations {
        for id in vec_ids {
            ret_val
                ...
                .or_insert(HashMap::from([(locale.clone(), f(&id, &locale).unwrap())]));
        }
    }
...
pub fn create_chapter_heuristic(
    ...
    Box::new(move |typed_id: &TypedId, locale: &Locale| {
        if get_collectability(typed_id) != Collectability::Collectable
            && get_chapter(typed_id).is_some_and(|x| x == binding)
        {
            let lang_id = LanguageIdentifier::from(locale);
            let str = locales.lookup(&lang_id, &binding2);
            Some(str.clone())
...
fn get_collectability(typed_id: &TypedId) -> Collectability {
    match typed_id.0 {
        ...
        PageType::KeyPage => get_key_page_by_id(&typed_id.1).map(|x| x.collectability.clone()),
        PageType::Passive => get_passive_by_id(&typed_id.1).map(|x| x.collectability.clone()),
    }
    .unwrap()
}
...
fn get_display_names_for_locale(locale: &Locale) -> HashMap<TypedId, String> {
    get_all_abno_pages()
        .into_iter()
        .map(|x| x.get_typed_id())
        .chain(
            get_all_battle_symbols()
                .into_iter()
                .map(|x| x.get_typed_id()),
        )
        .chain(get_all_combat_pages().into_iter().map(|x| x.get_typed_id()))
        .chain(get_all_key_pages().into_iter().map(|x| x.get_typed_id()))
        .chain(get_all_passives().into_iter().map(|x| x.get_typed_id()))
        .map(|x| {
            (
                x.clone(),
                get_display_equivalence(
                    get_display_names(&x).get(locale).unwrap_or(&x.to_string()),
                ),
            )
        })
        .collect()
...
mod tests {
    ...
    fn sanity_display_equivalence() {
        assert_eq!(
            get_display_equivalence("Xiao’s Page"),
            get_display_equivalence("Xiao")
        );
        assert_eq!(
            get_display_equivalence(",D@;Q7Yのページ"),
            get_display_equivalence(",D@;Q7Y")
        );
        assert_eq!(
            get_display_equivalence("enlxmfflsdis"),
            get_display_equivalence("enlxmfflsdis")
        );
    }
