...
pub fn reserialize_combat_page_locales(document_strings: &HashMap<Locale, Vec<String>>) -> String {
    let combat_pages: HashMap<Locale, HashMap<CombatPageLocaleKey, CombatPageLocaleValue>> =
        document_strings
            .iter()
            .map(|(x, y)| {
                (
...
fn process_combat_page_locale_file(
    document_string: &str,
) -> HashMap<CombatPageLocaleKey, CombatPageLocaleValue> {
...
fn parse_combat_page_locale(node: Node) -> (CombatPageLocaleKey, CombatPageLocaleValue) {
    ...
    let card_effect = serialize_option_2(
        get_unique_node_text(node, "Ability"),
    ...
    let dice_desc_overrides_vec: Vec<(usize, &str)> = get_nodes(node, "Behaviour")
        ...
        .map(|x| {
            (
                x.attribute("ID").unwrap().parse().unwrap(),
                x.text().unwrap_or(""),
            )
        })
        .collect();
