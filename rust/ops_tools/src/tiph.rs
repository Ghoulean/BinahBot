...
pub async fn decode(
    ...
    let txt = client
        .post(format!(
            "{}{}{}",
            BASE_TIPH_URL, "/internal/dvi_decode/?d=", tiph.0
        ))
...
pub async fn _encode(
    ...
    Ok(TiphDeck(
        serde_json::from_str::<TiphDeckEncode>(&txt)?.val,
        1,
    ))
}
...
impl TryFrom<&TiphDeckDecode> for DeckData {
    ...
    fn try_from(value: &TiphDeckDecode) -> Result<Self, Self::Error> {
        let mut resized_combat_page_ids: Vec<_> = value
            .data
            .cards
            .iter()
...
mod tests {
    ...
    async fn sanity_decode() {
        ...
        let decode = decode(&client, &turbo_nikolai)
            .await
        ...
        assert_eq!(
            decode.passive_ids,
            vec!["230018", "240118", "250025", "250151"]
        );
    }
    ...
    async fn sanity_encode() {
        ...
        let turbo_nikolai = DeckData {
            ...
            passive_ids: ["230018", "240118", "250025", "250151"]
                .iter()
                .map(|x| x.to_string())
                .collect(),
            combat_page_ids: [
        ...
        let encode = _encode(&client, &turbo_nikolai)
            .await
            .expect("error with encode");
