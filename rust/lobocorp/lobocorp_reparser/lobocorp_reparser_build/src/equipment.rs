...
const DEBUG_IDS: &[u32] = &[
    177777, 277777, // god weapon + armor
    4,      // rabbit team
    5, 11, 3, 200, 22, // leftover test equipment
    101, 100018, 100105, 100046, 100036, 1021, 1022,
    100041,  // special abno equipment from possessions and such
    1200001, // ordeals
];
...
pub fn load_equipment() -> AllEquipment {
    ...
    let equipment_list_node =
        get_unique_node(&doc.root(), "equipment_list").expect("couldn't find equipment_list");

    let mut weapons = Vec::new();
    let mut suits = Vec::new();
    let mut gifts = Vec::new();

    get_nodes(&equipment_list_node, "equipment")
        .iter()
        .for_each(|x| {
            let id: u32 = x
                .attribute("id")
...
fn parse_weapon(node: &Node) -> PartialWeapon {
    let id: u32 = node
        ...
        .parse()
        .expect("id parse failure");
    let name_id = get_unique_node_text(node, "name")
        .map(|x| x.to_string())
        .ok();
    let desc_id = get_unique_node_text(node, "desc")
        .map(|x| x.to_string())
        .ok();
    let special_desc_id = get_unique_node_text(node, "specialDesc")
        .map(|x| x.to_string())
        .ok();
    let risk = RiskLevel::try_from(
        get_unique_node_text(node, "grade")
            .expect("no grade")
            .parse::<i32>()
            .expect("couldn't parse grade"),
    )
    .expect("couldn't get risk level");
    let range = WeaponRange(
        get_unique_node_text(node, "range")
            .expect("no range")
            .parse::<u32>()
            .expect("couldn't parse weapon range"),
    );
    let attack_speed = get_unique_node_text(node, "attackSpeed")
        .ok()
    ...
    let damage_range = DamageRange(
        damage_node
            ...
            .expect("couldn't get min damage"),
        damage_node
            .attribute("max")
            .and_then(|x| x.trim().parse().ok())
            .expect("couldn't get max damage"),
    );

    let damage_type = if id == 200004 || id == 200038 {
        // In the name of love and hate, Twilight
        ...
        WeaponDamageType::Of(
            damage_node
                ...
                .expect("couldn't get damage type"),
        )
    };

    let max_collectable_amount = get_unique_node_text(node, "maxNum")
        .ok()
        .and_then(|x| x.parse::<i32>().ok())
        .unwrap_or(i32::MAX);

    let equip_requirements = get_nodes(node, "require")
        .iter()
        .map(|x| {
            let key = x
                .attribute("type")
                .and_then(|x| EquipRequirementKey::try_from(x).ok())
                .expect("no type attribute on requirement");
            let val = x
                .text()
                .and_then(|x| x.parse::<i32>().ok())
                .expect("couldn't get requirement val");
...
fn parse_suit(node: &Node) -> PartialSuit {
    ...
    let special_desc_id = get_unique_node_text(node, "specialDesc")
        .map(|x| x.to_string())
        .ok();
    let armor_id = get_unique_node_text(node, "armorId")
        .map(|x| x.to_string())
        .expect("couldn't find armorId");

    let defense_node = get_unique_node(node, "defense").expect("no defense node");
    let defenses = if id == 300015 {
        // Paradise Lost
        Defenses {
            red: Resistance(0.5),
    ...
    let risk = get_unique_node_text(node, "grade")
        .ok()
        .and_then(|x| x.parse::<i32>().ok())
...
pub fn get_defense_val(defense_node: &Node, str_code: &str) -> Resistance {
    Resistance(
        find_unique_node_with_attribute(&defense_node, "type", str_code)
            .ok()
            .and_then(|x| x.text())
            .and_then(|x| x.parse::<f64>().ok())
...
fn parse_gift(node: &Node) -> PartialGift {
    ...
    let stat_bonuses = bonus_node
        .map(|x| {
            x.descendants()
                .filter(|n| n.is_element() && n.tag_name().name() != "bonus")
                .map(|x| {
                    let key =
                        Stat::try_from(x.tag_name().name()).expect("couldn't recognize tag name");
                    let val = x
                        .text()
                        .and_then(|x| x.parse::<i32>().ok())
                        .expect("couldn't get val");
                    StatBonus(key, val)
                })
                .collect::<Vec<_>>()
        })
...
fn get_gift_slot(attach_pos: &str, attach_type: &str) -> Result<Slot, String> {
    Ok(
        match (
            attach_pos.to_lowercase().as_str(),
            attach_type.to_lowercase().as_str(),
        ) {
