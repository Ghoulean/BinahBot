...
pub fn find_unique_node_with_name_and_attribute<'a>(
    ...
    node.descendants().into_iter().filter(|x| {
        x.is_element() && x.has_tag_name(name)
    }).find(|x| {
        x.attribute(attribute).is_some_and(|x| x == attribute_val)
    }).ok_or("no node found")
}
...
pub fn find_unique_node_with_attribute<'a>(
    ...
    node.descendants().into_iter().find(|x| x.attribute(attribute).is_some_and(|x| x == attribute_val)).ok_or("no node found")
}
