...
pub fn load_encyclopedia_list() -> Vec<ListEntry> {
    ...
    let creature_list_node =
        get_unique_node(&doc.root(), "creature_list").expect("couldn't find creature_list");
    ...
    get_nodes(&creature_list_node, "creature")
        .iter()
        .map(|x| ListEntry {
            ...
            id: x
                ...
                .parse()
                .expect("failed parse into u32"),
            stat: get_unique_node_text(x, "stat")
                .expect("no stat text")
                .to_string(),
        })
        .chain(apo_bird)
